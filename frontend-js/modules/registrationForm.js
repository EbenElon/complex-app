...
export default class RegistrationForm {
    ...
    emailAfterDelay() {
        if (!/^\S+@\S+$/.test(this.email.value)) {
            this.showValidationError(this.email, 'You must provide valid email address.')
        }
    ...
    usernameImmediately() {
        ...
        if (this.username.value.length > 30) {
            this.showValidationError(this.username, 'Username connot exceeds 30 characters')
        }
    ...
    usernameAfterDelay() {
        ...
        if (!this.username.errors) {
            axios.post('/doesUsernameExist', { _csrf: this._csrf, username: this.username.value }).then((response) => {
                if (response.data) {
                    this.showValidationError(this.username, ' That Username is already taking! try different one')
